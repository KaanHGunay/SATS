...
{
    /// <summary>
    /// İstatistiği istenen mahallenin verilerinin gösterildiği form
    /// </summary>
    public static class MahalleIstatistikGoruntuleyici
    {
        public static void Olustur(Grid g, int mahalleID, DateTime? basTarih, DateTime? bitTarih)
        {
            g.Children.Clear();
            ...
            try
            {
                ...
                {
                    var sonuc = (from s in db.olaylar where s.mahalle.ID == mahalleID && s.tarih >= basTarih && s.tarih <= bitTarih select s);
