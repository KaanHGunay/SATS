...
{
    /// <summary>
    /// İstatistiği istenen ilin verilerinin gösterildiği form
    /// </summary>
    public static class IlIstatistikGoruntuleyici
    {
        public static void Olustur(Grid g, int ilID, DateTime? basTarih, DateTime? bitTarih)
        {
            g.Children.Clear();
            ...
            try
            {
                ...
                {
                    var sonuc = (from s in db.olaylar where s.mahalle.polisMerkezi.ilce.İl.ID == ilID && s.tarih >= basTarih && s.tarih <= bitTarih select s);
                    ...
                    var enCokIslenenSuc = ((from s in sonuc group s by s.suc into a select new { suc = a.Key, sayi = a.Count() }).OrderByDescending(c => c.sayi).ToList());
                    if (enCokIslenenSuc.Count() > 0)
                    {
                        lbl_enCokIslenenSuc.Content = String.Format("{0} ({1})", enCokIslenenSuc[0].suc.adi, enCokIslenenSuc[0].sayi);
                    }
                    else
                    {
                        lbl_enCokIslenenSuc.Content = "- (-)";
                    }
                }
            }
            catch (Exception ex)
